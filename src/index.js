...
nextAction.onPlayerChangedEvent = (e) => {
  ...
  || e.detail.level !== nextActionOverlay.playerObject.level) {
    ...
    nextActionOverlay.castingRegex = new RegExp(`^.{15}(?<logType>00:0[\\da-f]{3}):You (?:begin casting|ready) (?<actionName>${castingNames})\\.`, 'i');
    nextActionOverlay.cancelRegex = new RegExp(`^.{15}(?<logType>00:0[\\da-f]{3}):You cancel (?<actionName>${castingNames})\\.`, 'i');

    // This regex is always a static match right now
    nextActionOverlay.playerstatsRegex = new RegExp('^.{15}0C:Player Stats: (?<jobID>[\\d]+):(?<strength>[\\d]+):(?<dexterity>[\\d]+):(?<vitality>[\\d]+):(?<intelligence>[\\d]+):(?<mind>[\\d]+):(?<piety>[\\d]+):(?<attackPower>[\\d]+):(?<directHitRate>[\\d]+):(?<criticalHit>[\\d]+):(?<attackMagicPotency>[\\d]+):(?<healingMagicPotency>[\\d]+):(?<determination>[\\d]+):(?<skillSpeed>[\\d]+):(?<spellSpeed>[\\d]+):0:(?<tenacity>[\\d]+)'); // eslint-disable-line prefer-regex-literals
