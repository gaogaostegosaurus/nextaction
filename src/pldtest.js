...
const pldNextWeaponskill = ({
  ...
  } else if (player.level >= 54 && comboStep === 'Riot Blade' && goringbladeStatus < player.gcd) {
    return 'Goring Blade';
...
const pldNextOGCD = ({
  ...
  && fightorflightStatus < player.gcd) {
    return 'Requiescat';
  }
...
const pldNext = ({
  ...
  do {
    ...
    if (gcdTime <= 0 && spellTime === 0) {
      ...
      } else if (pldWeaponskills.includes(nextGCD)) {
        comboStep = nextGCD;
      ...
      /* Calculate gcd time */
      gcdTime = player.gcd;
      if (pldSpells.includes(nextGCD) && player.level < 78) {
        spellTime = 2500;
        gcdTime = 2500;
      } else {
        spellTime = 0;
  ...
  clearTimeout(timeout.pldNext);
  timeout.pldNext = setTimeout(pldNext, 14000);
};
